...
export function CopyToClipboard({ text, className, children }: CopyToClipboardProps) {
  ...
  return (
    ...
    >
      ...
      {copied && (
        <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[length:var(--text-2xs)] font-bold px-2 py-1 rounded shadow-lg whitespace-nowrap animate-in fade-in zoom-in duration-200">
          تم النسخ!
        </span>
      )}
    </div>
