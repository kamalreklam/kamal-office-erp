...
function ProductTypeIcon({ product, className = "size-5" }: { product: Product; className?: string }) {
    ...
    <div className="pb-32 bg-gradient-to-br from-indigo-50/50 via-white to-blue-50/30 min-h-screen" dir="rtl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16 space-y-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">مجموعة جديدة</h1>
            <p className="text-sm font-bold text-slate-500 mt-0.5">إنشاء وتكوين حزمة منتجات جديدة لإضافتها للفواتير بنقرة واحدة</p>
          </div>
        </div>
            ...
            <div className="bg-white rounded-[2rem] p-6 md:p-8 border border-slate-100 shadow-sm space-y-6">
              ...
              {Object.keys(inkFamilies).length > 0 && (
                <div className="space-y-4 p-5 bg-indigo-50/50 border border-indigo-100 rounded-2xl">
                  <span className="text-sm text-indigo-900 font-bold flex items-center gap-2">
                    <Droplet className="size-4 text-indigo-500" />
                    إضافة طقم أحبار كامل:
                  </span>
                  <div className="flex flex-wrap gap-2">
                    {Object.entries(inkFamilies).map(([cat, fam]) => (
                      <button
                        ...
                        className="px-4 py-2 text-sm font-bold bg-white text-indigo-700 border border-indigo-200 rounded-xl hover:bg-indigo-600 hover:text-white hover:border-indigo-600 shadow-sm transition-all active:scale-95"
                      >
                        {cat}
                      </button>
                    ))}
              ...
              <div className="p-6 bg-slate-50/50 rounded-2xl border border-slate-100 space-y-4">
                ...
                {resolved.length === 0 ? (
                  <div className="text-center py-6 text-sm font-bold text-slate-400 border border-dashed border-slate-200 rounded-xl bg-white">
                    قم بإضافة منتجات لترى التكلفة الإجمالية
                  </div>
                ) : (
